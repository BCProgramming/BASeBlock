...
namespace BASeBlock.Particles
{
    ...
    public abstract class Particle : BaseParticle
    {
        ...
        {
            float speedmult = gamestate!=null?gamestate.GetMultiplier():1;
        ...
        public abstract override Particle  CreateParticle(cBall EmitterBall);
    ...
    {
        //Particle class; base class for particles.

        /*
        public delegate bool ParticlePerformFrameDelegate(BCBlockGameState gstate,Particle theparticle);
        public delegate bool ParticleDrawDelegate(Graphics g, Particle theparticle);

        public event ParticlePerformFrameDelegate ParticleBeforePerformFrame;
        public event ParticlePerformFrameDelegate ParticleAfterPerformFrame;
        public event ParticleDrawDelegate ParticleBeforeDraw;
        public event ParticleDrawDelegate ParticleAfterDraw;

        #region event invokers

        public bool InvokeBeforePerformFrame(BCBlockGameState gstate)
        {
            ParticlePerformFrameDelegate ppfd = ParticleBeforePerformFrame;
            bool anytrue=false;
            if (ppfd != null)
            {
                anytrue |= ppfd.Invoke(gstate, this);
        ...
        public bool InvokeAfterPerformFrame(BCBlockGameState gstate)
        {
            ParticlePerformFrameDelegate ppfd = ParticleAfterPerformFrame;
            bool anytrue = false;
            if (ppfd != null)
            {
                anytrue |= ppfd.Invoke(gstate, this);
        ...
        public bool InvokeBeforeDraw(Graphics g)
        {
            ParticleDrawDelegate ppfd = ParticleBeforeDraw;
            bool anytrue = false;
            if (ppfd != null)
            {
                anytrue |= ppfd.Invoke(g,this);
        ...
        public bool InvokeAfterDraw(Graphics g)
        {
            ParticleDrawDelegate ppfd = ParticleAfterDraw;
            bool anytrue = false;
            if (ppfd != null)
            {
                anytrue |= ppfd.Invoke(g, this);
        ...
        */
        ...
        public abstract bool PerformFrame(BCBlockGameState gamestate);
        ...
        public abstract Particle CreateParticle(cBall EmitterBall);
    ...
    public class AnimatedImageParticle :SizedParticle 
    {
        ...
        {
            return BCBlockGameState.ClampValue((int)((((float)_LiveTime)/(float)_TTL) * _ImageFrames.Length)
        ...
        public override Particle CreateParticle(cBall EmitterBall)
        {
            return new AnimatedImageParticle(EmitterBall.Location)
            {TTL=45,Velocity=BCBlockGameState.VaryVelocity(new PointF(0,-1),Math.PI/10) };
        }
        ...
        public AnimatedImageParticle(PointF pLocation)
            : this(pLocation,BCBlockGameState.Imageman.getImageFrames("smoke"),45)
        {
    ...
    {
        protected static Image _BubbleImage = BCBlockGameState.Imageman.getLoadedImage("bubble");

        public BubbleParticle(PointF Location):base(Location,_BubbleImage,new SizeF(16,16))
        {
            //Velocity = new PointF(0,(float)BCBlockGameState.rgen.NextDouble()*-0.5f);
        ...
        public override Particle CreateParticle(cBall EmitterBall)
        {
            return new BubbleParticle(EmitterBall.Location, EmitterBall.Velocity);
        }
    ...
    public class CharacterDebris : PolyDebris
    {
        ...
        private Font _useFont = new Font(BCBlockGameState.GetMonospaceFont(), 14);
        ...
        protected override void GenPoly(double RadiusMin, double RadiusMax, int minpoints, int maxpoints)
        {
            ...
            Font buildfont = new Font (_useFont.Name,(float)BCBlockGameState.rgen.NextDouble(RadiusMin,RadiusMax));
            ...
            PointF gotcenter = BCBlockGameState.CenterPoint(PolyPoints);
    ...
    public class QuadDebris : Particle
    {
        private SizeF DrawSize;
        private Image SourceImage;
        private RectangleF SourceRect;
        //private float GravityEffect = 0.1f;
        public QuadDebris(PointF Location, PointF Velocity, SizeF pdrawsize,Image psourceimage, RectangleF psourcerect):base(Location)
        {
            base.Velocity=Velocity;
            DrawSize=pdrawsize;
            SourceImage=psourceimage;
            SourceRect = psourcerect;
            _GravityAcceleration = new PointF(0, 0.1f);

        }
        private static RectangleF Randomrect(Size rectsize)
        {
            Random rm = BCBlockGameState.rgen;
            return new RectangleF((float)rm.NextDouble()*(rectsize.Width-16),
                (float)rm.NextDouble()*(rectsize.Height-16),16,16);
                
                

        }

        public QuadDebris(cBall EmitterBall)
            : this(EmitterBall.Location, new PointF(0, 0), new SizeF(3, 3), BCBlockGameState.Imageman.getLoadedImage("crazyblock"), Randomrect(BCBlockGameState.Imageman.getLoadedImage("crazyblock").Size))
        {
            _GravityAcceleration= new PointF(0,0.1f);
        ...
        public override Particle CreateParticle(cBall EmitterBall)
        {
            return new QuadDebris(EmitterBall);
        }
        public override bool PerformFrame(BCBlockGameState gamestate)
        {
            base.PerformFrame(gamestate);
            ...
            //gravity...
            //Location = new PointF(Location.X+Velocity.X,Location.Y+Velocity.Y);
            //Velocity = new PointF(Velocity.X,Velocity.Y+GravityEffect);
        ...
        public override void Draw(Graphics g)
        {
            g.DrawImage(SourceImage, new RectangleF(Location.X, Location.Y, 4, 4), SourceRect, GraphicsUnit.Pixel);
        }
    ...
    public class BrickDebris : PolyDebris
    {
        ...
        /// <returns></returns>
        public static BrickDebris[] GenerateBricksFromBlock(int cols,int rows,Block sourceblock, PointF? BallSpeed, float RandomFactor)
        {
            //parameter checks.
            ...
            Block.DrawBlock(sourceblock, out BlockImage, out usecanvas);
            ...
            {
                PointF usevel = BCBlockGameState.GetRandomVelocity(RandomFactor);


                createdebris[i] = new BrickDebris(creationarray[i], BlockImage, new PointF(properoffset[i].Left, properoffset[i].Top), usevel, (float)(rg.NextDouble() - 0.5));
        ...
        public static BrickDebris[] GenerateQuadBricks(Block sourceblock, PointF BallSpeed)
        {
            return GenerateQuadBricks(sourceblock, BallSpeed, 2);
        ...
        public static BrickDebris[] GenerateQuadBricks(Block sourceblock,PointF? BallSpeed,float RandomFactor)
        {
            if(BallSpeed==null) BallSpeed = PointF.Empty;
...
//            PointF[] Velocitymultiplier = new PointF[] { new PointF(-1, -1), new PointF(1, -1), new PointF(-1, 1), new PointF(1, 1) }; 
            ...
            {
                PointF randomvel = BCBlockGameState.GetRandomVelocity(RandomFactor);
        ...
        private void initRect(RectangleF rectangleobj)
        {
            ...
            Location = rectangleobj.CenterPoint();
    ...
    public class PolyDebris : Particle
    {
        ...
        {
            double angle = BCBlockGameState.rgen.NextDouble() * (Math.PI * 2);
        ...
        public PolyDebris(PointF pLocation, float speed, Image ppieceimage, double RadiusMin, double RadiusMax, int MinPoints, int MaxPoints, Point? ClipTopLeft, Size? ClipSize)
            :this(pLocation,BCBlockGameState.GetRandomVelocity(speed),ppieceimage,RadiusMin,RadiusMax,MinPoints,MaxPoints,ClipTopLeft, ClipSize)
        {
            ...
            : base(pLocation)
        {
            ...
            Random rr = BCBlockGameState.rgen;
        ...
        public static IEnumerable<PolyDebris> Fragment(iImagable source,int numfragments,Func<float> SpeedFunc )
        {
            ...
            if(SpeedFunc==null) SpeedFunc = (()=>(float)((BCBlockGameState.rgen.NextDouble()*2)+1));
            ...
            Image useimage = BCBlockGameState.ScaleImage(createimage, 3);
            ...
            PointF usevelocity = BCBlockGameState.GetRandomVelocity(SpeedFunc());
            for (int i=0;i<numfragments;i++)
            {
                ...
                Point clipspot = new Point(BCBlockGameState.rgen.Next((int) (source.Size.Width/2)),BCBlockGameState.rgen.Next((int) (source.Size.Width/2)));
                PolyDebris newdebris = new PolyDebris(source.getRectangle().CenterPoint(),usevelocity,useimage,minradius,maxradius,3,8,new Point?(clipspot),new Size?(ClipSize));
        ...
        public PolyDebris(cBall Emitter)
            : this(Emitter.Location, new PointF(0, 0), Color.Red)
        {
            //if (Emitter.DrawBrush is SolidBrush)
            //    mDustColor = (Emitter.DrawBrush as SolidBrush).Color;

            Random rGen = BCBlockGameState.rgen;    

            Location = new PointF(Emitter.Location.X+(float)((Emitter.Radius*2)*rGen.NextDouble()*2)-(Emitter.Radius*2),
                Emitter.Location.Y + (float)((Emitter.Radius * 2) * rGen.NextDouble() * 2) - (Emitter.Radius * 2));
            //Velocity = new PointF(Velocity.X-Emitter.Velocity.X,Velocity.Y-Emitter.Velocity.Y);


        }
         public PolyDebris(cBall Emitter, Image useimage)
             : this(Emitter.Location, new PointF(0, 0), useimage)
         {
             Random rGen = BCBlockGameState.rgen;
        ...
        public override void Draw(Graphics g)
        {
            ...
            try
            {
                ...
                {
                    Alpha = BCBlockGameState.ClampValue(Alpha, 0, 255);
        ...
        public override Particle CreateParticle(cBall EmitterBall)
        {
            return new PolyDebris(EmitterBall);
        }
    }
    ...
    public class LineParticle : Particle
    {
        ...
        private TimeSpan getAliveTime { get
        {
        ...
        private int CalculateAlpha()
        {
            ...
            return BCBlockGameState.ClampValue(255 - (int)(255f * percentage), 0, 255);
        ...
        public LineParticle(Particle EndPointA, Particle EndPointB, Color linecolor)
            : base(BCBlockGameState.MidPoint(EndPointA.Location, EndPointB.Location))
        {
            ParticleA = EndPointA;
            ParticleB = EndPointB;
            usecolor=true;
        ...
        public LineParticle(Particle EndPointA, Particle EndPointB, Pen LinePen):base(BCBlockGameState.MidPoint(EndPointA.Location,EndPointB.Location))
            {
...
public override bool  PerformFrame(BCBlockGameState gamestate)
{
    ...
    Location = BCBlockGameState.MidPoint(ParticleA.Location,ParticleB.Location);
...
public override Particle  CreateParticle(cBall EmitterBall)
{
    ...
    public class DebrisParticle : Particle
    {
        ...
        public DebrisParticle(cBall Emitter):base(Emitter.Location)
        {
            //if (Emitter.DrawBrush is SolidBrush)
            //    mDustColor = (Emitter.DrawBrush as SolidBrush).Color;
        ...
        public static Particle CreateDebris(cBall EmitterBall)
        {

         
            Random mrgen = BCBlockGameState.rgen;
            return new DebrisParticle(BCBlockGameState.Imageman.getImageFrames("debris"), EmitterBall.Location, EmitterBall.Velocity, (float)mrgen.NextDouble() * (Math.PI * 2), (float)mrgen.NextDouble());
    ...
    public class WaterParticle : Particle
    {
        ...
        private static Random rGen = BCBlockGameState.rgen;
        ...
        public WaterParticle(cBall Emitter):this(Emitter.Location,DefBaseColour)
        {
            Location = new PointF(Emitter.Location.X + (float)((Emitter.Radius ) * rGen.NextDouble() ) - (Emitter.Radius ),
              Emitter.Location.Y + (float)((Emitter.Radius ) * rGen.NextDouble()) - (Emitter.Radius ));
            mVelocity = new PointF(mVelocity.X-Emitter.Velocity.X,mVelocity.Y-Emitter.Velocity.Y);
        ...
        public override bool PerformFrame(BCBlockGameState gamestate)
        {
            ...
            //BCBlockGameState.IncrementLocation(gamestate, ref Location, mVelocity);
        ...
        public override Particle CreateParticle(cBall Emitter)
        {
            //

            return new WaterParticle(Emitter);
        }
    }
    ...
    public class DustParticle : Particle
    {
        ...
        private static Random rGen { get { return BCBlockGameState.rgen; } }
        ...
        public DustParticle(PointF pLocation, float maxspeed):base(pLocation)
            {
                ...
                Velocity = new PointF((float)(BCBlockGameState.rgen.NextDouble() * maxspeed) - halfmax, (float)(BCBlockGameState.rgen.NextDouble() * maxspeed) - halfmax);
            }
        ...
        {
            Velocity = new PointF((float)(BCBlockGameState.rgen.NextDouble() * 2) - 1, (float)(BCBlockGameState.rgen.NextDouble() * 2) - 1);


        }
        public DustParticle(cBall Emitter, int TTL)
            : this(Emitter)
        {
            minitialAliveFrames=TTL;
            mAliveFrames=TTL;
        ...
        public DustParticle(cBall Emitter):base(Emitter.Location)
        {
            //if (Emitter.DrawBrush is SolidBrush)
            //    mDustColor = (Emitter.DrawBrush as SolidBrush).Color;
        ...
        public override Particle CreateParticle(cBall EmitterBall)
        {
            return new DustParticle(EmitterBall);
        }
    }
    public class Sparkle : Particle
    {
        ...
        */
        ...
        public override Particle CreateParticle(cBall EmitterBall)
        {
            return new Sparkle(EmitterBall.Location, EmitterBall.Velocity);
        }
    }
    ...
    public class FireParticle : Particle
    {
        ...
        public int TTL = 15;
        public FireParticle(cBall Emitter) : this(Emitter.Location)
        {
        ...
        {
            Velocity = new PointF((float)(BCBlockGameState.rgen.NextDouble() * .4f) - .2f, (float)(BCBlockGameState.rgen.NextDouble() * .4f) - .2f);
        ...
        public override Particle CreateParticle(cBall EmitterBall)
        {
            return new FireParticle(EmitterBall);
        }
    }
    /// <summary>
    ...
    public class EmitterParticle : Particle
    {
        ...
        public delegate Particle EmissionRoutine(BCBlockGameState gstate,EmitterParticle Source,int FrameNum,int TTL);
        private Random rGen = BCBlockGameState.rgen;
        protected EmissionRoutine EmissionFunction;
        public EmitterParticle(cBall Emitter, EmissionRoutine emitroutine):base(Emitter.Location)
        {
        ...
        private bool proxyaddparticle(ProxyObject sourceobject, BCBlockGameState gamestate)
        {
            Particle addit = sourceobject.Tag as Particle;
            if (addit != null)
                gamestate.Particles.Add(addit);
        ...
        public override bool PerformFrame(BCBlockGameState gamestate)
        {
            TTL--;
            FrameNum++;
            Particle addme = EmissionFunction(gamestate,this,FrameNum, TTL);
            ProxyObject addproxy = new ProxyObject(proxyaddparticle, null);
            addproxy.Tag= addme;

            gamestate.GameObjects.AddLast(addproxy);

            return base.PerformFrame(gamestate) || TTL <0;
        }
        private Particle DefaultEmissionRoutine(BCBlockGameState gs, EmitterParticle Source,int FrameNum, int TTL)
        {
        ...
        public override Particle CreateParticle(cBall EmitterBall)
        {
            return new EmitterParticle(EmitterBall.Location, DefaultEmissionRoutine);
        }
    ...
    public class GenericEmitter<T>:EmitterParticle  where T:Particle
    {
        ...
        public override Particle CreateParticle(cBall EmitterBall)
        {
            return new GenericEmitter<T>(EmitterBall.Location);
        }
    ...
    public class LightOrb : Particle
    {
        ...
        public override bool PerformFrame(BCBlockGameState gamestate)
        {
            ...
            return base.PerformFrame(gamestate) || LiveTime >= TTL;
        }
        public override Particle CreateParticle(cBall EmitterBall)
        {
            return new LightOrb(EmitterBall.Location, EmitterBall.DrawColor, 15);
        }
        ...
        public override void Draw(Graphics g)
        {
            ...
            pgb.CenterPoint = BCBlockGameState.CenterPoint(glr);
