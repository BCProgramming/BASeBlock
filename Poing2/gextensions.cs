...
namespace BASeCamp.BASeBlock
{
    static partial class GraphicsExtension
    {
        ...
        public static void DrawPieRad(this Graphics g, Pen usepen, float x, float y, float width, float height, float startAngle, float sweepAngle)
        {




            g.DrawPie(usepen, x, y, width, height, startAngle, sweepAngle);



        }
        public static void DrawLineRandom(this Graphics g, PointF Start, PointF End, Pen DrawPen)
        {

            g.DrawLineRandom(Start,End,DrawPen,5,8);

        }
        public static PointF getPercentAlongLine(PointF Start, PointF End, float Percentage)
        {

            PointF Difference = new PointF(End.X - Start.X, End.Y - Start.Y);
            return new PointF(Start.X + Difference.X * Percentage, Start.Y + (Difference.Y * Percentage));

        }
        public static void DrawLineRandom(this Graphics g, PointF Start, PointF End, Pen DrawPen,int numlines,float warble)
        {

            Random rgen = BCBlockGameState.rgen;


            Func<float,float> getWarble = ((a)=>a*(float)rgen.NextDouble());
            //first, make sure we make a line at the start and end.
            g.DrawLine(DrawPen, Start, End);
            for (int i = 0; i < numlines; i++)
            {
                PointF randomstart = getPercentAlongLine(Start, End, (float)rgen.NextDouble());
                PointF randomEnd = getPercentAlongLine(Start,End,(float)rgen.NextDouble());

                randomstart = new PointF(randomstart.X + getWarble(warble), randomstart.Y + getWarble(warble));
                randomEnd = new PointF(randomEnd.X + getWarble(warble), randomEnd.Y + getWarble(warble));

                g.DrawLine(DrawPen, randomstart, randomEnd);
        ...
        public static void DrawStringCenter(this Graphics g, String stringdraw, Font usefont, Brush usebrush, Pen usePen, RectangleF within, Point Offset)
        {
            SizeF stringsize = g.MeasureString(stringdraw, usefont);

            Rectangle userect = BCBlockGameState.CenterRect(within, stringsize);
            userect.Offset(Offset);
            GraphicsPath p = new GraphicsPath();
            StringFormat Centerformat = new StringFormat();
            Centerformat.Alignment = StringAlignment.Center;
            p.AddString(stringdraw, usefont.FontFamily, (int)usefont.Style, usefont.Size, userect.Location, Centerformat);
            g.FillPath(usebrush, p);
            g.DrawPath(usePen, p);
